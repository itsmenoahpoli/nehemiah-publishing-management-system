 ...
 */
router.get("/sales", auth_1.authenticateToken, auth_1.requireAnyRole, async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
router.get("/inventory", auth_1.authenticateToken, auth_1.requireAnyRole, async (req, res) => {
    try {
        ...
        const totalWarehouseBooks = warehouseStocks.reduce((sum, stock) => sum + stock.quantity, 0);
        const totalSchoolBooks = schoolStocks.reduce((sum, stock) => sum + stock.quantity, 0);
        res.json({
            success: true,
            data: {
                warehouseStocks,
...
router.get("/transactions", auth_1.authenticateToken, auth_1.requireAnyRole, async (req, res) => {
    try {
        ...
        if (type === "all" || type === "school") {
            ...
            transactions.push(...schoolTransactions.map((trans) => ({ ...trans, type: "school" })));
        }
        if (type === "all" || type === "returns") {
            ...
            transactions.push(...returns.map((ret) => ({ ...ret, type: "return" })));
        }
...
router.get("/schools", auth_1.authenticateToken, auth_1.requireAnyRole, async (req, res) => {
    try {
