...
router.get("/overview", async (req, res) => {
    try {
        const [totalBooks, totalSchools, totalCustomers, totalUsers, pendingRegistrations, totalBills, totalTransactions, totalReturns, totalWarehouseStock, totalSchoolStock] = await Promise.all([
        ...
        const [pendingBills, paidBills, cancelledBills, pendingTransactions, completedTransactions, cancelledTransactions, pendingReturns, approvedReturns, rejectedReturns] = await Promise.all([
...
router.get("/charts/books", async (req, res) => {
    try {
        ...
        const bookDetails = await Promise.all(topSellingBooks.map(async (book) => {
...
router.get("/charts/inventory", async (req, res) => {
    try {
        ...
        const schoolStockDetails = await Promise.all(schoolStock.map(async (stock) => {
...
router.get("/charts/schools", async (req, res) => {
    try {
        ...
        const schoolsWithDetails = await Promise.all(schoolsByStock.map(async (school) => {
...
router.get("/recent-activity", async (req, res) => {
    try {
        const [recentBills, recentTransactions, recentReturns, recentRegistrations] = await Promise.all([
